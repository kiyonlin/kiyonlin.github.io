...
[演示和容器组件](https://medium.com/@dan_abramov/smart-and-dumb-components-7ca2f9a7c7d0)
...
- 演示组件 - 这些组件只是将数据作为输入，并知道如何在屏幕上显示它。 他们还可以发出自定义事件
...
演示组件将负责获取输入数据并将其呈现给用户。 例如，`course-detail-header`是一个表示组件。 我们来看看这个组件的样子：
```ts
@Component({
...
## 演示组件更深一层组件树
新闻通讯组件也是一个表示组件，因为它需要输入，显示订阅表单并在订阅时发出事件：
```ts
@Component({
    selector: 'newsletter',
    template: `
